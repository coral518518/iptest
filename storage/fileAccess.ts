...
export const SAVED_IPS_FILEPATH = `${APP_DIR}/saved_ips.json` as const;
...
export const initDefaultFilesIfNeeded = async () => {
  ...
  const isSavedIpsExist = await FileSystem.exists(SAVED_IPS_FILEPATH);
  if (!isSavedIpsExist) {
    await FileSystem.writeFile(SAVED_IPS_FILEPATH, "{}", "utf8");
  }
};
